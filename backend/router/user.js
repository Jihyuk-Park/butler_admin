...
router.get('/userInfo/getResult/all/:page', function(req,res){
...
router.get('/userInfo/getResult/search/:page/:searchType/:searchInput', function(req,res){
  let page = req.params.page;
...
router.get('/userInfo/getTotalNum/all', function(req,res){
  ...
  connection.query(sql, function(err, rows, fields){
    if (err){
      ...
      res.send(rows[0]);
      // console.log('전체-페이지네이션 글 개수 카운트 완료', rows[0]);
    }
  })
});
...
router.get('/userInfo/getTotalNum/search/:searchType/:searchInput', function(req,res){
  ...
  connection.query(sql, function(err, rows, fields){
    if (err){
      ...
      res.send(rows[0]);
      // console.log('검색-페이지네이션 글 개수 카운트 완료', rows[0]);
    }
  })
});
