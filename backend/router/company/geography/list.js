...
router.get('/getData/all/:page/:sortField/:sortType', function(req,res){
  ...
  let sql = `SELECT a.corp_name, a.corp_code, a.stock_code, IF(b.commit_type="ANALYSIS", max(b.last_commit_date), NULL) as recent, geography_last_updated, geography_title1, geography_title2, geography_source, currency, unit, is_available FROM CompanyInfo a 
...
router.get('/getData/search/:searchCompanyCode', function(req,res){
  ...
  let sql = `SELECT a.corp_name, a.stock_code, a.corp_code, max(b.updated_at) as recent, geography_last_updated, geography_title1, geography_title2, geography_source, currency, unit, is_available FROM CompanyInfo a 
    LEFT JOIN company_last_commit b on a.corp_code = b.corp_code
    LEFT JOIN analysis_company_info c on a.corp_code = c.corp_code
