...
export default function CommonBuyback() {
  ...
  useEffect(() => {
    if (searchCorpCode !== 'main') {
      axios
        .get(
          `${url}/admin/company/otherInfo/buyback/getData/search/rawReports/common/${searchCorpCode}`,
        )
        .then(result => {
  ...
  return (
    <>
      {buybackData.map(function (eachdata, index) {
        return (
          <StyledTableRow
            key={`보통주${buybackAccountArray[index][1]}${buybackAccountArray[index][0]}`}
          >
            <StyledTableCell
