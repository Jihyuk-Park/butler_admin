...
export default function EditAddSalesAndProfitModal({
  ...
  const dataTable = ['지역1', '지역2', '지역3', '원본 데이터', editDate];
  ...
  const saveData = () => {
    ...
    if (body.unit === '') {
      ...
      axios
        .post(
          `/admin/company/geography/individual/performance/${type}/edit/${searchCorpCode}/${year}/${quarter}/${unit}`,
          body,
        )
        .then(() => {
          alert('수정이 완료되었습니다');
