...
export default function AddEditInputGeographyModal({
  addEditModalSwitch,
  ...
  const saveData = () => {
    ...
    if (isDuplicate === 1) {
      ...
      alert('추가하기를 이용해주세요(수정 - 하위지역 추가 알림)');
    } else {
      // 지역1 검사
      ...
      if (body.depth1 === '') {
        ...
        if (body.depth2 === '' && body.depth3 !== '') {
          ...
          axios
            .post(`${url}/admin/company/geography/individual/info/geography/${mode}`, body)
            .then(() => {
  ...
  const checkDuplicate = () => {
    ...
    if (isEditModal) {
      checkDuplicateArray = geographyInfoData.filter(
        data =>
  ...
  const checkDepthDuplicate = () => {
    ...
    if (editInput.depth2 !== '' && editInput.depth3 === '') {
      const checkDepth1DuplicateArray = geographyInfoData.filter(
      ...
      const checkDepth2DuplicateArray = geographyInfoData.filter(
...
AddEditInputGeographyModal.defaultProps = {
